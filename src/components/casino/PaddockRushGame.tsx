...
export const PaddockRushGame: React.FC<GameProps> = ({ balance, onUpdateBalance, addLog }) => {
  ...
  const tickCrashGame = () => {
    ...
    if (currentMulti >= crashLimitRef.current) {
      ...
      setCommentary(`💥 TRIP-COLLAPSE! Mascot stumbled at ${crashLimitRef.current.toFixed(2)}x! Bet lost.`);
      addLog("Paddock Rush", safeStake, 0, "LOSS", `Crashed at ${crashLimitRef.current.toFixed(2)}x`);
    } else {
  ...
  const handleCashout = () => {
    ...
    const winVal = safeStake * finalOdds;
    onUpdateBalance(balance - safeStake + winVal);
    setCommentary(`💰 CASHED OUT at ${finalOdds.toFixed(2)}x! Total return $${winVal.toFixed(2)}!`);
    addLog("Paddock Rush", safeStake, finalOdds, "WIN", `Cashed out at ${finalOdds.toFixed(2)}x`);
  };
    ...
    <div className="space-y-4">
      <div className="bg-[#070b11] border border-white/5 rounded-2xl p-6 flex flex-col justify-between items-center relative overflow-hidden h-52 select-none">
        ...
        <div className="my-auto z-10 flex flex-col items-center">
          <span className={`font-mono text-5xl font-black tracking-tight transition-all ${
            isCrashed ? "text-red-500" : isCashedOut ? "text-emerald-400" : "text-amber-400 animate-pulse"
          }`}>
        ...
        <div className="w-full h-8 bg-black/60 border border-white/5 rounded-full flex items-center justify-between px-3 overflow-hidden relative z-10">
          <div className="h-full bg-emerald-500 transition-all rounded-full absolute left-0 top-0 duration-75" style={{ width: `${Math.min(100, (multiplier / 20) * 100)}%` }}></div>
          <span className="text-[10px] font-mono text-slate-300 z-10 uppercase font-bold">START 1.0x</span>
          <span className="text-[10px] font-mono text-amber-400 z-10 font-black">CRASH POINT</span>
        </div>
      </div>
      ...
      {!isLive && (
        <StakeSlider balance={balance} stake={safeStake} setStake={setStake} disabled={isLive} label="WAGER STAKE" />
      )}
