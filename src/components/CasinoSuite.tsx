...
export const CasinoSuite: React.FC<CasinoSuiteProps> = ({ balance, onUpdateBalance, username }) => {
  ...
  const [logs, setLogs] = useState<RollingLog[]>(() => {
    const saved = localStorage.getItem("fs_casino_logs_v5");
    if (saved) return JSON.parse(saved);
    return [
      { id: "1", game: "Penalty Shootout", timestamp: Date.now() - 300000, amount: 100, multiplier: 6.0, status: "WIN", details: "Scored Top-Left Volley" },
      { id: "2", game: "SportyMines", timestamp: Date.now() - 250000, amount: 50, multiplier: 2.1, status: "WIN", details: "Cleared 4 Helmets" },
  ...
  const saveLogs = (newLogs: RollingLog[]) => {
    setLogs(newLogs);
    localStorage.setItem("fs_casino_logs_v5", JSON.stringify(newLogs));
  };
  ...
  const gamesList = [
    { id: "redblack", name: "Red or Black Streak", rtp: "97.0%", desc: "Double or nothing 4-round streak. Beware of the trick Joker! Boosted up to 28x.", tag: "HOT STREAK", color: "from-red-500/20 to-black/40 border-red-500/30", multiplier: "up to 28.0x" },
    { id: "bottle", name: "Spin the Bottle", rtp: "97.5%", desc: "Bet Up or Down with the rotating champagne bottle. 2% center-freeze risk.", tag: "CLASSIC", color: "from-yellow-500/10 to-black/40 border-yellow-500/30", multiplier: "2.2x" },
    { id: "crash", name: "Paddock Rush", rtp: "97.2%", desc: "Predict how far the football mascot runs before tripping. Multiplier climbs exponentially!", tag: "HIGH VOLATILITY", color: "from-emerald-500/20 to-black/40 border-emerald-500/30", multiplier: "uncapped" },
    { id: "mines", name: "SportyMines", rtp: "98.0%", desc: "Configure custom mines on a 5x5 pitch. Uncover helmets and cash out early.", tag: "STRATEGY", color: "from-blue-500/20 to-black/40 border-blue-500/30", multiplier: "customizable" },
    { id: "shootout", name: "Penalty Shootout", rtp: "97.5%", desc: "Interactive spot kick simulation. Beat the keeper for massive 40x multipliers!", tag: "SKILL-BASED", color: "from-purple-500/20 to-black/40 border-purple-500/30", multiplier: "up to 40.0x" },
    { id: "slots", name: "Football Slots", rtp: "96.5%", desc: "Spin classic football reels with high-paying Cups (100x) and Golden Boots (50x).", tag: "CASUAL", color: "from-amber-600/20 to-black/40 border-amber-500/30", multiplier: "up to 100.0x" },
    { id: "plinko", name: "Golden Boot Plinko", rtp: "98.1%", desc: "Drop a golden chip through pegs into boosted high multiplier bins.", tag: "BEST RTP", color: "from-pink-500/20 to-black/40 border-pink-500/30", multiplier: "up to 15.0x" },
    { id: "dice", name: "Over / Under Dice", rtp: "97.8%", desc: "Roll high-fidelity duel dice. Adjust targets for boosted payouts.", tag: "SWIFT REELS", color: "from-sky-500/20 to-black/40 border-sky-500/30", multiplier: "up to 6.5x" }
  ];

  const currentGame = gamesList.find(g => g.id === activeGame);

  return (
    <div className="flex-1 min-h-0 overflow-y-auto bg-[#05070a] text-slate-100 flex flex-col no-scrollbar" id="sportsim-elite-casino-suite">
        ...
        <div className="mx-4 md:mx-6 mt-4 bg-red-500/10 border border-red-500/30 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 select-none">
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <p className="text-[10px] text-slate-300">Your wallet balance is ${balance.toFixed(2)}. Claim $500 free trial credits!</p>
            </div>
          </div>
          <button
            onClick={() => {
      ...
      <div className={`flex-1 grid gap-5 p-4 md:p-5 min-h-0 ${activeGame && isFullView ? "grid-cols-1" : "grid-cols-1 lg:grid-cols-12"}`}>
        ...
        {!(activeGame && isFullView) && (
          <div className={`${activeGame ? "hidden lg:flex lg:col-span-4" : "lg:col-span-4"} flex flex-col gap-4 min-h-0`}>
            <div className="bg-[#0b0e14] border border-white/5 rounded-2xl p-4 flex flex-col justify-between shrink-0 select-none">
              <div>
                <span className="text-[10px] font-mono font-bold tracking-widest text-slate-400 uppercase block">VIP MEMBERS CLUB</span>
                <h3 className="text-xs font-bold text-slate-100 font-sans mt-0.5">Hi, Manager {username}!</h3>
                <p className="text-[11px] text-slate-400 mt-2 leading-relaxed">
                  Wager between kickoff matches. All bets are logged to the live regional leaderboard profile automatically.
                </p>
              </div>
            ...
            <div className="flex-1 bg-[#0b0e14] border border-white/5 rounded-2xl p-4 flex flex-col min-h-0">
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar space-y-2 mt-3 max-h-[280px] lg:max-h-none">
                {logs.length === 0 ? (
                  <div className="text-center text-slate-500 text-[11px] py-12">No spins logged yet.</div>
                ) : (
                  logs.map(log => {
                    ...
                    return (
                      <div key={log.id} className="bg-black/25 border border-white/5 rounded-xl p-2.5 flex items-center justify-between text-xs hover:bg-slate-900/40 transition-all">
                        ...
                        <div className="text-right font-mono text-xs">
                          <span className={`font-black ${isWin ? "text-emerald-400" : isJoker ? "text-amber-500" : isFreeze ? "text-blue-400" : "text-red-400"}`}>
            ...
            <div className="flex-1 bg-[#0b0e14] border border-white/5 rounded-2xl flex flex-col min-h-0 relative select-none z-10 shadow-2xl overflow-hidden">
              ...
              <div className="flex items-center justify-between border-b border-white/5 px-4 py-3 shrink-0">
                <div className="flex gap-2">
                  <button
                    onClick={() => { setActiveGame(null); setIsFullView(false); }}
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right hidden sm:block">
                    ...
                    <span className="text-xs font-black text-slate-200 tracking-wide font-sans">{currentGame?.name}</span>
                  </div>
                  <div className="flex items-center gap-1.5 bg-black/50 border border-white/10 rounded-xl px-2.5 py-1.5">
                    ...
                    <div>
                      <span className="text-[8px] text-slate-500 block uppercase font-mono leading-none">BALANCE</span>
              ...
              {balance < 10 && balance >= 0 && (
                <div className="mx-4 mt-3 bg-red-500/10 border border-red-500/25 rounded-xl px-3 py-2 flex items-center justify-between gap-3 shrink-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-red-300 font-mono">Low balance! Claim emergency cash to keep playing.</span>
                  </div>
                  <button
                    onClick={() => { onUpdateBalance(balance + 500); addLog("Emergency Grant", 0, 0, "WIN", "Claimed $500 emergency fund"); }}
              ...
              <div className="flex-1 min-h-0 overflow-y-auto no-scrollbar p-4">
                {activeGame === "redblack" && (
                  <RedOrBlackGame balance={balance} onUpdateBalance={onUpdateBalance} addLog={addLog} />
                )}
                {activeGame === "bottle" && (
                  <SpinTheBottleGame balance={balance} onUpdateBalance={onUpdateBalance} addLog={addLog} />
                )}
                {activeGame === "crash" && (
                  <PaddockRushGame balance={balance} onUpdateBalance={onUpdateBalance} addLog={addLog} />
                )}
                {activeGame === "mines" && (
                  <SportyMinesGame balance={balance} onUpdateBalance={onUpdateBalance} addLog={addLog} />
                )}
                {activeGame === "shootout" && (
                  <PenaltyShootoutGame balance={balance} onUpdateBalance={onUpdateBalance} addLog={addLog} />
                )}
                {activeGame === "slots" && (
                  <FootballSlotsGame balance={balance} onUpdateBalance={onUpdateBalance} addLog={addLog} />
                )}
                {activeGame === "plinko" && (
                  <PlinkoGame balance={balance} onUpdateBalance={onUpdateBalance} addLog={addLog} />
                )}
                {activeGame === "dice" && (
                  <OverUnderDiceGame balance={balance} onUpdateBalance={onUpdateBalance} addLog={addLog} />
                )}
              </div>
            </div>
          ) : (
                ...
                {gamesList.map(g => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mt-4 pt-3 border-t border-white/5 text-[10px] font-mono text-slate-400">
                      <span>Max Multi: <b className="text-emerald-400">{g.multiplier}</b></span>
